...
const staffNotification = (
	...
	if (status === "accepted") {
		...
		additionalInfo =
			reason === null
				? "You may reapply in the future when staff applications are open."
	...
	return stripIndents`
        ...
        We are ${
					status === "accepted" ? "happy" : "sorry"
				} to notify you that your staff application has been ${status}. ${additionalInfo}
        
		Bit *on behalf of* root, the ownership team.
	`
}

const proficientNotification = (
	status: "accepted" | "denied",
	reason: string | null,
	usertag: string
) => {
	...
	if (status === "accepted") {
		...
		additionalInfo =
			reason === null
				? "You may reapply in the future when proficient applications are open."
				: `Reason for denial: ${reason}`
	}
	return stripIndents`
		Dear ${usertag},
		...
		We are ${
			status === "accepted" ? "happy" : "sorry"
		} to notify you that your proficient application has been ${status}. ${additionalInfo}
		
		Bit *on behalf of* root, the ownership team.
	`
}

const fluentNotification = (
	status: "accepted" | "denied",
	reason: string | null,
	usertag: string
) => {
	...
	if (status === "accepted") {
		...
		additionalInfo =
			reason === null
				? "You may reapply in the future when fluent applications are open."
				: `Reason for denial: ${reason}`
	}
	return stripIndents`
		Dear ${usertag},
		...
		We are ${
			status === "accepted" ? "happy" : "sorry"
		} to notify you that your fluent application has been ${status}. ${additionalInfo}
...
function generateApplicationMessage(
	type: "staff" | "proficient" | "fluent",
	status: "accepted" | "denied",
	reason: string | null,
	usertag: string
) {
