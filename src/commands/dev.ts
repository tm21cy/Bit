		...
		switch (subcommand) {
			case "restart": {
				...
				const embed = new EmbedBuilder()
					...
					.setDescription(
						`A manual restart has been issued by **${
							interaction.user.tag
						}** (\`${interaction.user.id}\`) at ${Util.formatTimestamp(
							"dateAndRelative",
							date.getTime()
						)}.`
					)
				...
				exec("pm2 restart bit", function (error, stdout, stderr) {
					interaction.reply({
			...
			case "shutdown": {
				...
				const embed = new EmbedBuilder()
					...
					.setDescription(
						`A manual shutdown has been issued by **${
							interaction.user.tag
						}** (\`${interaction.user.id}\`) at ${Util.formatTimestamp(
							"dateAndRelative",
							date.getTime()
				...
				channel.send({ embeds: [embed] })
				await interaction.reply({ content: "✅" })
				exec("pm2 stop bit", function (error, stdout, stderr) {
					interaction.reply({
						content: `stdout: ${stdout}\n\nstderr: ${stderr}`
					})
					if (error !== null) {
