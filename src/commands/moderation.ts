...
import { ActionRowBuilder, AttachmentBuilder, ButtonBuilder, ButtonInteraction, ButtonStyle, ChatInputCommandInteraction, Embed, EmbedBuilder, Guild, GuildBan, Interaction, PermissionFlagsBits, SlashCommandBuilder, TextChannel, User } from "discord.js"
import followRedirect from "follow-redirect-url"
import { client } from ".."
import Sentry from "../services/sentry"
import Util from "../utilities/general"
import re2 from "re2"
import { FriskyDetailedResponse, FriskyFormattedDetailedResponse } from "../types/Apis"
...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand
				...
				.setDescription("Run this command to stop all further joins and pause invites.")
		)
		.addSubcommandGroup((group) =>
			group
				.setName("join-alert")
				.setDescription("Manages the user join alert system which posts a log when the specified user joins the server.")
				.addSubcommand((subcommand) =>
					subcommand
						.setName("add")
						...
						.addUserOption((option) =>
							option
								.setName("user")
								.setDescription("The user to add to the join alert list. Supports IDs.")
								.setRequired(true)
						)
				)
				.addSubcommand((subcommand) =>
					subcommand
						...
						.addUserOption((option) =>
							option
								.setName("user")
								.setDescription("The user to remove from the join alert list. Supports IDs.")
								.setRequired(true)
						)
				)
				.addSubcommand((subcommand) =>
	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		let confirmRow = new ActionRowBuilder<ButtonBuilder>()
			.addComponents(
				new ButtonBuilder()
					.setCustomId("confirm")
					.setLabel("Confirm")
		...
		const confirmEmbed = new EmbedBuilder()
			...
			.setDescription(`Are you sure you would like to enable undetected raid mode?`)
			.setColor(Colors.Red)
		...
		switch (subcommand) {
			case "follow-url":
				if (!url) return interaction.reply({ content: "You must provide a URL." })
				followRedirect.startFollowing(url)
					.then((res: [{
						url: string,
						redirect: boolean,
						status: number,
				...
				break
			case "scan":
				if (!type) return interaction.reply({ content: "You must provide a type." })
				switch (type) {
					case "usernames":
						if (!query) return interaction.reply({ content: "You must provide a query." })
					...
					case "accounts":
						...
						let friskyScammers: FriskyDetailedResponse | FriskyFormattedDetailedResponse | Error = await Sentry.getBlacklistedUsers()
						if (friskyScammers instanceof Error) {
							interaction.editReply({
								content: `An error occurred while scanning: ${codeBlock("js", friskyScammers)}`,
							})
							return
						}
				...
				break
			case "whois":
				if (!user) return interaction.reply({ content: "You must provide a user." })
				interaction.reply({
					content: "",
			...
			case "pause-invites":
						...
						function undetectedRaid(interaction: ChatInputCommandInteraction, i: ButtonInteraction) {
							i.update({
								content: "Processing...",
								embeds: [],
								components: [],
							...
							const alertsChannel = guild.channels.cache.get("933381980488335390") as TextChannel
							const noticeEmbed = new EmbedBuilder()
								.setTitle(`${interaction.user.tag} has enabled raid mode.`)
								.setDescription(stripIndents`
									**Auto-actions taken:**
										- ${pausedInvites ? "Invites paused, no accounts will be able to join." : "Failed to pause invites."}
								`)
								.setColor(Colors.Red)
								...
								.addFields([
									{
										...
										value: "Look at the ID join logs and try to find the time the raid accounts started joining."
									},
									{
										name: "2. Get the IDs of the raid accounts",
										value: "Drag-select all IDs from the first raid account to the last raid account. Then copy to clipboard"
									},
									{
		...
		async function infoUser(id: string) {
			...
			const inGuild = await interaction.guild?.members.fetch(id)
				.catch(() => false)
			const banned = await interaction.guild?.bans.fetch(id)
				.then((ban: GuildBan) => {
		...
		async function pauseInvites(executioner: User, Guild: Guild) {
			...
			return await axios.patch(`https://discord.com/api/v10/guilds/${Guild.id}`, data, { headers: headers })
				.then((res) => {
