...
import {
	...
	EmbedBuilder,
	// rome-ignore lint/correctness/noUnusedVariables: used on line 642
	Guild,
	GuildBan,
	Interaction,
	PermissionFlagsBits,
	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		switch (subcommand) {
				...
				switch (type) {
					...
					case "accounts":
						...
						await interaction.reply({
							content: `Scanning ${
								members.size
							} members. Started ${Util.generateTimestamp("R", Date.now())}.`
						})
						let friskyScammers:
							| FriskyDetailedResponse
							| FriskyFormattedDetailedResponse
						...
						if (friskyScammers instanceof Error) {
							log.error(friskyScammers, "An error occurred while scanning.")
							interaction.editReply({
			...
			case "pause-invites":
				...
				collector?.on("end", async (_x, reason) => {
					if (reason === "time") {
						interaction.editReply({
							content: "Timed out.",
							embeds: [],
			...
			case "join-alert":
				switch (subcommand) {
					case "add":
						...
						Query.joinAlerts
							.isUserJoinAlert(interaction.guildId, joinAlertUser.id)
							.then((res) => {
								...
								Query.joinAlerts
									...
									.then(() => {
										interaction.reply({
											content: `Added join alert for ${joinAlertUser.toString()} (${
												joinAlertUser.tag
											}, ${joinAlertUser.id})`
										})
									})
					...
					case "remove":
						...
						Query.joinAlerts
							...
							.then((res) => {
								...
								Query.joinAlerts
									.removeUserJoinAlert(interaction.guildId, joinAlertUser.id)
									.then((res) => {
					...
					case "list":
						Query.joinAlerts
							.retrieveUserJoinAlerts(interaction.guildId)
							.then(async (res) => {
								if (res.status === Status.NOTFOUND) {
									return interaction.reply({
								...
								const list = (res.data as JoinAlertOutput[]).map((alert) => {
									const inGuildEmoji = interaction.guild.members.cache.has(
										alert.target_id
									)
										? "<:red_shield:1043294200529166386>"
										: ""
