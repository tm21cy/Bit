...
module.exports = {
	...
	async execute(member: GuildMember) {
		...
		const joinIdLogChannel = await member.guild.channels.fetch(
			"928779076225871903"
		) // #join-id-log
		if (!joinIdLogChannel?.isTextBased()) return

		const created = moment(member.user.createdTimestamp).fromNow()
		const createdButtonStyle =
			member.user.createdTimestamp > Date.now() - 1209600000
				? ButtonStyle.Danger
				: ButtonStyle.Success
		const userJoinAlert = await Query.joinAlerts.isUserJoinAlert(
			member.guild.id,
			member.user.id
		)
		const alertButtonStyle =
			userJoinAlert.status === Status.NOTFOUND
				? ButtonStyle.Success
				: ButtonStyle.Danger
		...
		if (userJoinAlert.status !== (Status.NOTFOUND || Status.ERROR)) {
			const joinAlertChannel = await member.guild.channels.fetch(
				"933381980488335390"
			) // #alerts
			...
			const moderator = await member.client.users.fetch(
				(userJoinAlert.data as JoinAlertOutput).moderator_id
			)
			...
			const alertEmbed = new EmbedBuilder()
				...
				.addFields({
					name: "Reason",
					value:
						(userJoinAlert.data as JoinAlertOutput).reason ??
						"No reason provided"
				})
